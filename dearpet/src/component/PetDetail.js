...
const PetDetail = () => {
  ...
  const [healthAdvice, setHealthAdvice] =
    useState("정보를 가져오는 중입니다...");
  ...
  useEffect(() => {
    const fetchPets = async () => {
      try {
        const response = await axios.get("https://3.34.219.248/api/pets", {
          headers: {
    ...
    try {
      if (isEditMode && editingPetData) {
        const response = await axios.patch(
          `https://3.34.219.248/api/pets/${editingPetData.petId}`,
          newPetData,
          {
            headers: {
              Authorization: `Bearer ${localStorage.getItem("token")}`,
        ...
        setPets(
          pets.map((pet) =>
            pet.petId === editingPetData.petId ? response.data : pet
          )
        );
        setEditingPetData(null);
      } else {
        const response = await axios.post(
          "https://3.34.219.248/api/pets",
          newPetData,
          {
            headers: {
              Authorization: `Bearer ${localStorage.getItem("token")}`,
  ...
  const handleDeletePet = async (petId) => {
    try {
      await axios.delete(`https://3.34.219.248/api/pets/${petId}`, {
        headers: {
          Authorization: `Bearer ${localStorage.getItem("token")}`,
        },
      });
  ...
  const getHealthAdvice = async (petId) => {
    try {
      const response = await axios.get(`https://3.34.219.248/api/pets/advice`, {
        headers: {
          Authorization: `Bearer ${localStorage.getItem("token")}`,
        },
        params: { petId: petId },
  ...
  return (
    ...
    >
      ...
      {pets.map((pet) => (
        <Card
          key={pet.petId}
          onClick={() => handlePetModalOpen(pet)}
          style={{
            margin: "10px 0",
            width: "100%",
            maxWidth: 800,
        ...
        >
          <CardContent sx={{ flexGrow: 1, marginLeft: "1vw" }}>
            ...
            <Typography>
              <span style={{ color: "gray", marginRight: "30px" }}>종</span>{" "}
              {pet.species}
            </Typography>
            <Typography>
              <span style={{ color: "gray", marginRight: "30px" }}>나이</span>{" "}
              {pet.age} 살
            </Typography>
            <Typography>
              <span style={{ color: "gray", marginRight: "30px" }}>성별</span>{" "}
              {pet.gender === "MALE" ? "남아" : "여아"}
            </Typography>
            <Typography>
              <span style={{ color: "gray", marginRight: "13px" }}>중성화</span>{" "}
              {pet.neutered ? "O" : "X"}
            </Typography>
            <Typography>
              <span style={{ color: "gray", marginRight: "13px" }}>몸무게</span>{" "}
              {pet.weight} kg
            </Typography>
            <Typography>
              <span style={{ color: "gray", marginRight: "13px" }}>
                건강상태
              </span>{" "}
              {pet.healthStatus}
            </Typography>
            <Box
              sx={{ display: "flex", justifyContent: "center", marginLeft: 15 }}
            >
              <Button
                onClick={(e) => {
                  e.stopPropagation();
      ...
      <Modal open={openPetModal} onClose={handlePetModalClose}>
        ...
        >
          ...
          <FormControlLabel
            control={
              <Checkbox
                ...
                onChange={(e) =>
                  setPetData({ ...petData, neutered: e.target.checked })
                }
                sx={{
                  color: "#6A47B1",
                  "&.Mui-checked": {
          ...
          <TextField
            ...
            onChange={(e) =>
              setPetData({ ...petData, healthStatus: e.target.value })
            }
            sx={{
              marginBottom: 2,
              "& .MuiOutlinedInput-root": {
      ...
      <Modal open={openPetDetail} onClose={handlePetDetailClose}>
        ...
        >
          <h3 style={{ textAlign: "center" }}>
            {selectedPet ? `${selectedPet.name}는 어떤 특성이 있나요?` : ""}
          </h3>
          <IconButton
          ...
          <Typography sx={{ fontWeight: "bold" }}>
            우리 {selectedPet ? selectedPet.name : ""}는...
          </Typography>
          <Box
            sx={{
              minHeight: "10vh",
          ...
          >
            <Typography>
              {loading ? "정보를 가져오는 중입니다..." : healthAdvice}
            </Typography>
          </Box>
        </Box>
      </Modal>
