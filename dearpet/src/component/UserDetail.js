...
const UserDetail = () => {
  ...
  const handleCheckId = async () => {
    try {
      const response = await axios.get(`http://3.34.219.248:8080/api/auth/check-username?username=${formData.username}`);
  ...
  if (formData.newPassword !== formData.confirmNewPassword) {
    ...
    try {
      await axios.patch('http://3.34.219.248:8080/api/profile', updatedData, {
        headers: {
          Authorization: `Bearer ${localStorage.getItem('token')}`,
        },
      });
