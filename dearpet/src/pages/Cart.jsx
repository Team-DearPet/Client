...
const Cart = () => {
  ...
  const deleteitem = async (id) => {
    try {
      const accessToken = localStorage.getItem("token");
      const response = await fetch(
        `https://3.34.219.248/api/cart/items/${id}`,
        {
          method: "DELETE",
          headers: {
            "Content-Type": "application/json",
  ...
  const getTotalOrderAmount = () => {
    return items
      ...
      .reduce((sum, item) => {
        const itemPrice =
          prices.find((price) => price.itemId === item.cartItemId)?.price || 0;
  ...
  const handleQuantityChange = async (id, delta) => {
    const newQuantity =
      items.find((item) => item.cartItemId === id).quantity + delta;
        ...
        >
          ...
          {items.map((item) => (
            <CartItem
              ...
              item={item}
              totalPrice={
                prices.find((price) => price.itemId === item.cartItemId)
                  ?.price || 0
              }
              handleTotalChange={(newPrice) =>
                handleTotalChange(item.cartItemId, newPrice)
              }
