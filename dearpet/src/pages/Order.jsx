...
const Order = () => {
  ...
  const fetchUser = async () => {
    const response = await fetch("https://3.34.219.248/api/profile", {
      method: "GET",
      headers: {
  ...
  const proceedPay = () => {
    ...
    $.ajax({
      url: "https://3.34.219.248/api/verification/prepare",
      type: "POST",
      async: true,
      dataType: "json",
  ...
  const requestPay = (data) => {
    ...
    IMP.request_pay(
      ...
      function (rsp) {
        if (rsp.success) {
          $.ajax({
            url: "https://3.34.219.248/api/verification/confirm",
            method: "POST",
            dataType: "json",
            contentType: "application/json",
            headers: {
  ...
  const succeedPay = (impUid, merchantUid) => {
    fetch("https://3.34.219.248/api/payments/save", {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
        Authorization: `Bearer ${accessToken}`,
      ...
      .then((data) => {
        ...
        openDialog("결제 정보가 성공적으로 저장되었습니다!", () =>
  ...
  const cartCheckout = async (impUid) => {
    try {
      ...
      const baseUrl = `https://3.34.219.248/api/orders/checkout?impUid=${impUid}`;
      const url =
      ...
      <Container maxWidth={false} sx={{ padding: 3, width: "80%" }}>
        <Box sx={{ maxWidth: 800, margin: "auto", paddingLeft: "20px" }}>
          ...
          <ShippingInfo
            ...
            onRequireChange={setRequirements}
          />
          <OrderSummary
