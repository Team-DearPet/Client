...
const OrderHistory = () => {
      ...
      const orders = await Promise.all(
        data.map(async (order) => {
          const itemsResponse = await fetch(
            `https://3.34.219.248/api/orders/${order.orderId}/items`,
            {
              method: "GET",
              headers: {
                "Content-Type": "application/json",
          ...
          const items = await Promise.all(
            itemsData.map(async (item) => {
              const productResponse = await fetch(
                `https://3.34.219.248/api/products/${item.productId}`,
                {
                  method: "GET",
                  headers: {
                    "Content-Type": "application/json",
  ...
  const orderCancel = async (id) => {
    try {
      const response = await fetch(`https://3.34.219.248/api/orders/${id}`, {
        method: "PATCH",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${accessToken}`,
    ...
    <div>
              ...
              >
                <Typography
                  sx={{
                    ...
                    color: statusFilter === status ? "#7B52E1" : "black",
                  }}
                >
                  {status}
                </Typography>
                <Typography
                  sx={{ color: statusFilter === status ? "#ff4d4f" : "gray" }}
                >
                  {statusCounts[status]}
                </Typography>
              </div>
      ...
      >
        ...
        <DialogActions sx={{ mr: "15px", mb: "15px" }}>
          <Button onClick={handleDialogClose} sx={{ color: "#6A47B1" }}>
            취소
          </Button>
          <Button
            onClick={() => {
