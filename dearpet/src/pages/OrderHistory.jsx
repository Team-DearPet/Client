...
const OrderHistory = () => {
  ...
  const fetchOrders = async () => {
    try {
      const response = await fetch(`http://3.34.219.248:8080/api/orders`, {
        method: 'GET',
        headers: {
      ...
      const orders = await Promise.all(
        data.map(async (order) => {
          const itemsResponse = await fetch(`http://3.34.219.248:8080/api/orders/${order.orderId}/items`, {
            method: 'GET',
            headers: {
              'Content-Type': 'application/json',
              'Authorization': `Bearer ${accessToken}`,
          ...
          const items = await Promise.all(
            itemsData.map(async (item) => {
              const productResponse = await fetch(`http://3.34.219.248:8080/api/products/${item.productId}`, {
                method: 'GET',
                headers: {
                  'Content-Type': 'application/json',
                  'Authorization': `Bearer ${accessToken}`,
...
useEffect(()=>{fetchOrders()},[])
  ...
  const orderCancel = async (id) => {
    try {
      const response = await fetch(`http://3.34.219.248:8080/api/orders/${id}`, {
        method: 'PATCH',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${accessToken}`,
