    ...
    try {
      ...
      const orders = await Promise.all(
        data.map(async (order) => {
          const itemsResponse = await fetch(`https://3.34.219.248/api/orders/${order.orderId}/items`, {
            method: 'GET',
            headers: {
              'Content-Type': 'application/json',
              'Authorization': `Bearer ${accessToken}`,
          ...
          const items = await Promise.all(
            itemsData.map(async (item) => {
              const productResponse = await fetch(`https://3.34.219.248/api/products/${item.productId}`, {
                method: 'GET',
                headers: {
                  'Content-Type': 'application/json',
                  'Authorization': `Bearer ${accessToken}`,
