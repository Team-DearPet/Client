...
export default function SignUp() {
  ...
  const checkUsernameAvailability = async () => {
    try {
      const response = await fetch(`https://3.34.219.248/api/auth/check-username?username=${id}`);
  ...
  const handleEmailVerification = async () => {
    ...
    try {
      const response = await fetch("https://3.34.219.248/api/auth/send-verification-code", {
        method: 'POST',
        headers: {
